      ...
      <div class="near-building-box">
        ...
        <div class="info-near-building">
          ...
          <div class="info-right">
            <h3 class="building-price"><?php echo $list->e_br_currency;?> <?php echo $list->f_br_typical;?></h3>
          </div>
        </div>
...
<script src="https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=true"></script>  <!-- Google Maps API -->
 <script>
  ...
  function Init()
  {
    ...
    // Default to Washington, DC
      ...
      ShowLocation( -6.2143828,106.8432097);
  }
  
  // Callback function for asynchronous call to HTML5 geolocation
  function UserLocation( position )
  {
    ShowLocation( position.coords.latitude, position.coords.longitude, "You Are Here" );
  }
...
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function(position) {
        ...
        var geolocation = new google.maps.Marker({
            ...
            title: 'Your Are Here',
            icon: 'http://cdn.flaticon.com/png/64/22976.png'
        });
    });
}
    var locations = [
      <?php foreach ($building as $list):
      $alamat = json_encode($list->s_location);
      $currency = json_encode($list->e_br_currency);
      $price = json_encode(number_format($list->f_br_typical));
    ?>
    ['<div class="infowindow-onmap"><div class="img-onmap"><img src="<?php if($list->s_picture){
                      $img_src = 'uploads/building/image/'.$list->s_picture;
                      $width = 100;
                      $height = 75;
                      echo base_url('/uploads/building/image/'. thumb($img_src, $width, $height));
                    ...
                    } ?>" width="100" height="75"></div><div class="detail-onmap"><a href="<?php echo base_url();?>building/detail/<?php $name = $list->s_building_name;
                 $lowcaps = strtolower($name);
                 $underscr = str_replace(' ', '-', $lowcaps);
                  echo $underscr;?>-<?php echo $list->pk_building_id;?>" ><h1 class="building-name-onmap"><?php echo $list->s_building_name;?></h1></a><h3 class="building-location-onmap"><i class="fa fa-map-marker"></i><?php echo str_replace('"',' ',addslashes($alamat));?></h3><div class="price-onmap"><?php echo str_replace('"',' ',addslashes($currency))." ".str_replace('"',' ',addslashes($price)); ?></div><div class="viewdetail-onmap"><a class="link-viewdetail" href="<?php echo base_url();?>building/detail/<?php $name = $list->s_building_name;
                 $lowcaps = strtolower($name);
                 $underscr = str_replace(' ', '-', $lowcaps);
                  echo $underscr;?>-<?php echo $list->pk_building_id;?>">Detail</a></div></div>', <?php echo $list->s_lat;?>, <?php echo $list->s_lng;?>], 
    <?php endforeach;?> 
    ];
    ...
    <script src="<?php echo base_url();?>assets/ipapa/js/jquery-ui.js"></script>
      ...
      $(document).ready(function() {
        ...
        <script type="text/javascript">
          ...
          $.ajax({
          type: "POST",
          url : "<?php echo site_url('building/select_city')?>",
          data: province_id,
